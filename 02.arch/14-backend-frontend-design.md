...
| NestJS API | 3002 / 17012 | `03.source/beckend` (예정) |
...
FOTA Lite 스크립트 참고: `start-local.sh`, `local-up.sh` — 테크밸리는 `90.infra/local/` Podman compose SSOT.
...
| 항목 | 상태 |
|------|------|
| 프론트 UI + mock + DataScope | **done** (`03.source/frontend`) |
| Lambda pipeline skeleton | **skeleton** (`03.source/lambda`) — [15-lambda-development.md](./15-lambda-development.md) |
| NestJS `03.source/beckend/` | **next** — FOTA Lite contexts 포팅 |
| Cognito + API Gateway | **next** |
